...
{
	error::ErrorHandling::Assert(pos < m_children.size(), "GetChild(): Bounds error.", __FILE__, __LINE__);
...
{
	error::ErrorHandling::Assert(pos < m_children.size(), "RemoveChild(): Bounds error.", __FILE__, __LINE__);
...
unsigned int Node::GetChildPosition(Node* child) const
{
  for(unsigned int i = 0; i < m_children.size(); i++) 
	...
	error::ErrorHandling::Assert(false, "GeChildPosition(): Unknown son.", __FILE__, __LINE__);
